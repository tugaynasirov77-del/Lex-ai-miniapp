...
- Welcome → Quick Project Setup → Decoder → AdaptedTopics → PersonalScript → Save → Plan → Post-publish — end-to-end собран.
- **Единая визуальная система:** IG-гамма (`#A24FD6→#E84B91→#F88A4A`) везде, жёлтый `#F5E70A` вычищен; эмодзи → line-иконки (кроме `StateBlock`). Детали → [[40_Development/CHANGELOG]] (2026-06-21).
- Home перестроен по воронке (приветствие, «Что создаём сегодня?», «Продолжить работу», план-сводка).
- Метрики: ~282 юзера, 0 активных платящих в БД. Главная цель — конверсия в платёж.
- Свежие фичи (21.06): перенос материала в плане (§18), вилка/усиление идеи в «Сценарий с нуля» (§14), метрики после публикации (§20), синхронизация тогла уведомлений (§19).

> [!warning] Launch-блокеры закрыты по КОДУ, но есть OPS-хвосты
> 1. **ЮKassa webhook** починен (был критический баг — 0 активаций). Проверить webhook-URL в дашборде + восстановить покупателя 19.06 (payment `31c72bcf-000f-5000-b000-15d7fe2ef2f2`, нужен `tg_id`).
> 2. **Уведомления:** `/api/cron/tick` не в `vercel.json` — нужен внешний планировщик (hourly), иначе reminders не шлются.
