...
Ты пишешь как ОПЫТНЫЙ АВТОР, не как AI. Живой, конкретный, без воды.

═══ ПРИНЦИПЫ ═══
1. Конкретика > абстракции. Цифры, имена, события, цитаты.
2. Hook в первой строке — цепляет за 3 секунды. Максимум 12 слов.
3. Структура: hook → 1-3 конкретных факта → инсайт → CTA (опц.).
4. Tone-of-voice бренда священен. Если есть референсные посты — копируй их интонацию, ритм, длину предложений.
5. Один пост = одна мысль. Не пытайся уместить три тезиса.

═══ КАТЕГОРИЧЕСКИЙ ЗАПРЕТ ═══
Эти слова и обороты — мгновенный признак AI-копирайтинга. НЕ ИСПОЛЬЗУЙ:
успех, развитие, эффективность, потенциал, синергия, оптимизация,
продуктивность, лидерство, ценность, качественный, многогранный,
важно отметить, в современном мире, на сегодняшний день, всем известно,
давайте поговорим, я хочу поделиться, в этом году, сегодня многие,
вместе мы, ключевой момент, неотъемлемая часть, играет важную роль,
открывает новые возможности.

Также НЕ начинай посты с: «Друзья!», «Привет!», «Сегодня поговорим о…»,
«Многие знают…», «Знаете ли вы…», «А вы задумывались…».
...
- ПРЯМОЙ ИНСАЙТ: «Главное не в этом»

═══ ОБЯЗАТЕЛЬНЫЕ ЭЛЕМЕНТЫ ═══
Каждый материал ДОЛЖЕН содержать хотя бы ОДНО:
- Конкретное число с единицей измерения (47%, 3 млн ₽, 8 минут)
- Имя реального человека/бренда/продукта
- Конкретное событие с датой/местом
- Прямая цитата

═══ ЭТАЛОННЫЕ ПОСТЫ (учись на этих) ═══

[ПРИМЕР-1, hook: цифра]
<b>За 4 месяца я потратил 380 000 ₽ на рекламу — и слил почти всё.</b>

Запускал Reels-офферы, таргет в нельзяграм, посевы. По метрикам всё ок:
CPM, CTR, охваты. Только заявок было в 6 раз меньше плана.

<blockquote>Проблема была не в рекламе. Я продавал то, что нужно мне, а не клиенту.</blockquote>

Перестроил оффер за 3 дня. Тот же бюджет стал давать заявки.

[ПРИМЕР-2, hook: парадокс]
<b>Уволил клиента, который платил 200к/мес. Лучшее решение за год.</b>

3 месяца он отнимал 60% времени команды. Бесконечные правки,
доделки по ночам, скандалы из-за каждой запятой.

<blockquote>Когда ушёл — освободились руки на двух новых. Каждый платит 150. Сплю как нормальный человек.</blockquote>

[ПРИМЕР-3, hook: вопрос]
<b>Почему ты до сих пор пишешь посты вручную?</b>

Знакомый предприниматель тратит 4 часа в неделю на 2 поста.
Это 16 часов в месяц. Полных два рабочих дня.

За эти же 16 часов он мог бы сделать 3 коммерческих созвона
и закрыть на 600к. Но он пишет посты.

═══ ФОРМАТИРОВАНИЕ ПОСТОВ (только writePost) ═══
- HOOK (первая строка) ОБЯЗАТЕЛЬНО в <b>...</b>
- Главный инсайт / цитата — в <blockquote>...</blockquote> РОВНО один раз
- <i>...</i> для 1-2 акцентов, редко
- ВСЁ остальное (p, br, h1, ul, li, span, div, strong, em, code) — ЗАПРЕЩЕНО
- Эмодзи: 0-2 на пост, по делу (не «🚀💡✨» подряд)
- Символы & < > вне тегов → &amp; &lt; &gt;

═══ ОТВЕТ ═══
ВСЕГДА — строгий JSON одной строкой по указанной в задаче схеме.
...
export type BrandKit = {
  ...
  reference_posts?: string[];   // 3-5 эталонных постов автора — для копирования tone-of-voice
};
...
function buildContextBlock(insights: LexInsights | null, brand: BrandKit | null): string {
  ...
  if (brand) {
    ...
    parts.push("");
    if (brand.reference_posts && brand.reference_posts.length > 0) {
      parts.push("ЭТАЛОННЫЕ ПОСТЫ АВТОРА (копируй интонацию, ритм, длину предложений, лексику):");
      brand.reference_posts.slice(0, 5).forEach((p, i) => {
        const trimmed = String(p).trim().slice(0, 800);
        parts.push(`[REF-${i + 1}]\n${trimmed}\n`);
      });
      parts.push("");
    }
...
// ---------- Method 2b: refinePost (iteration) ----------

export type RefineDirection =
  | "shorter"      // короче
  | "sharper"      // острее, провокационнее
  | "emotional"    // эмоциональнее, живее
  | "specific";    // конкретнее (цифры, имена, факты)

const REFINE_INSTRUCTIONS: Record<RefineDirection, string> = {
  shorter:
    "Сделай ЗАМЕТНО короче — убери всё что можно убрать без потери смысла. Минус 30-40% длины. Сохрани hook в <b> и инсайт в <blockquote>.",
  sharper:
    "Сделай ОСТРЕЕ. Убери смягчающие обороты, добавь провокацию. Hook должен бить наотмашь. Если есть осторожность — убери её. Цель: вызвать реакцию.",
  emotional:
    "Сделай ЖИВЕЕ и ЭМОЦИОНАЛЬНЕЕ. Добавь конкретную сцену, ощущение, физический момент. Меньше абстракции, больше «как было». Если можно — добавь короткую прямую речь.",
  specific:
    "Добавь КОНКРЕТИКИ. Замени общие слова на цифры (47%, 3 года, 380 000 ₽), имена, даты, события. Если в посте нет ни одной цифры — добавь минимум одну осмысленную.",
};

export async function refinePost(args: {
  client: Anthropic;
  projectId: string;
  tgId: number;
  currentPost: string;       // HTML-текст текущего поста
  direction: RefineDirection;
  insights: LexInsights | null;
  brand: BrandKit | null;
}): Promise<{ post: PostVariant | null; cost: number }> {
  const { client, projectId, tgId, currentPost, direction, insights, brand } = args;
  const ctx = buildContextBlock(insights, brand);
  const instruction = REFINE_INSTRUCTIONS[direction];

  const task = `ЗАДАЧА: переделай существующий пост в указанном направлении.
...
НАПРАВЛЕНИЕ ПЕРЕДЕЛКИ:
${instruction}

ТРЕБОВАНИЯ:
- HOOK в <b>...</b> в первой строке
- РОВНО один <blockquote>...</blockquote> с главной мыслью
- Длина body: 300-450 символов (без учёта тегов)
- Никаких других HTML-тегов
- Не теряй главную тему оригинала, меняй только подачу

Верни JSON:
{
  "hook": "первая строка нового поста",
  "body": "полный текст 300-450 симв с тегами",
  "title": "короткий заголовок 30-70 симв plain text"
}
...
Только JSON.`;
  ...
  const raw = extractText(res);
  const parsed = safeJson<PostVariant>(raw);
  if (!parsed || !parsed.body || parsed.body.length < 50) {
    return { post: null, cost };
  }
  return {
    post: {
      hook: String(parsed.hook ?? "").slice(0, 200),
      body: String(parsed.body).trim().slice(0, 600),
